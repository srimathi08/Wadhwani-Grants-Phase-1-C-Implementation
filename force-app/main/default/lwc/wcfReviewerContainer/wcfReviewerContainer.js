...
@track isReturnToApproverBusy  = false;
    ...
    _toastTimer = null;
        ...
        /**
         * Review form always shows once the record is ready, whether or not
         * the application was flagged. A flag no longer gates the form —
         * it's just informational (see showFlagNotice below).
         * Still hidden while an Approver-return is pending action.
         */
        get showReviewForm() {
            return !this.isReturnedByApprover;
        }
        ...
        // ─────────────────────────────────────────────────────────────────────────
        // FLAG INFO LOADER — read-only now, just fetches the criteria for display
        // ─────────────────────────────────────────────────────────────────────────
        async _loadFlagInfo() {
            try {
                ...
                this.otherConcern           = info.otherConcern || '';   // ← ADD THIS
        ...
        _resetFlagState() {
            ...
            this.otherConcern           = '';   // ← ADD THIS
        }

        // ─────────────────────────────────────────────────────────────────────────
        // APPROVER RETURN INFO LOADER (unchanged)
        // ─────────────────────────────────────────────────────────────────────────
    ...
    get toastClass() {
        ...
        // ── APPROVER RETURN ACTION HANDLERS (unchanged) ─────────────────────────
    ...
    async handleReviewerReturnToValidator() {
       ...
       this.isReturnToValidatorBusy = true;
        ...
        // ── PANE COLLAPSE HANDLERS (unchanged) ──────────────────────────────────
